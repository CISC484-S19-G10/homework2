...
def extract_instances(dir_name, class_value=None):
	...
	instances = [parse_inst(raw_inst) for raw_inst in raw_instances]
...
def train_perceptron(training_data, learning_rate=1/64, inital_weight=lambda x: 1, n_iters=100):
	...
	for i in range(n_iters):
		for inst in training_data:
			for key in weights:
				weights[key] += learning_rate * (instance[CLASS_VALUE] \
				                                 - perceptron_fun(instance, weights))

	#bind the weights to the generic perceptron function
	return lambda inst: perceptron_function(inst, weights)

def build_perceptron_classifier(class_dirs):
	#combine the data from each directory of example instances of a class
	data = []
	for class_value, dir_name in class_dirs.items():
		data.extend(extract_instances(dir_name, class_value))
